...
def laplacian(W, normalized=True):
    ...
    if W.shape[0] == 1:
        return scipy.sparse.csr.csr_matrix(0, dtype=W.dtype)
...
def restore_L(L, lmax=2):
    """Return the Laplacian eigenvalues in [0, 2]."""
    M, M = L.shape
    I = scipy.sparse.identity(M, format='csr', dtype=L.dtype)
    L += I
    L *= lmax / 2
...
def laplacian_renorm(W):
    """
    Return the Laplacian of the weight matrix
    with re-normalization trick for GCN.
    """
    ...
    AA = D * WW * D
    # assert np.abs(L - L.T).mean() < 1e-9
    #     assert type(AA) is scipy.sparse.csr.csr_matrix
    return AA
